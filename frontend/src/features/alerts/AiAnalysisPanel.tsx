...
export default function AiAnalysisPanel({
    ...
    const handleContinueAnalysis = async () => {
        ...
        try {
            ...
            const response = await getApis().alerts.getGroomingAnalysisAlertsAlertIdAnalysisGet({
                alertId: alert.id
            });
    ...
    return (
        <div className="flex flex-col h-[calc(100vh-12rem)] bg-slate-900/40 border border-slate-800 rounded-xl overflow-hidden">
                    ...
                    <div className="space-y-2">
                        {GROOMING_STAGES_DETAILS.map((stage, idx) => {
                            ...
                            return (
                                <div
                                    key={idx}
                                    className={`border rounded-lg transition-all overflow-hidden ${isDetected
                                ...
                                >
                                    ...
                                    >
                                        <div className="flex items-center gap-2.5 min-w-0">
                                            {/* Number Icon */}
                                            <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold shrink-0 ${isDetected
            ...
            <div className="p-4 bg-slate-900/60 border-t border-slate-800 space-y-3">
                {/* State Indicator */}
                {analysisStatus !== "idle" && (
                    <div className={`p-2.5 rounded-lg border text-xs flex items-center gap-2 ${analysisStatus === "running"
